...
The bound tracks the proposer attempt duration, not the acceptor exclusion duration: TLC passes exhaustively with `Quarantine = ProposerDuration < AcceptorDuration` and violates lease exclusivity one unit below (the separated-duration model-checking experiment rules out the acceptor half of a `max(ProposerDuration, AcceptorDuration)` bound). The Python timing layer generalizes the bound to bounded local clock-rate error and operation margin. Note that the bound depends on the prepare-time timer rule; with the 2012 paper's step-3 rule no finite quarantine suffices.
...
| Proposer restart cannot reuse a ballot. | Python rule. | restart-component ballot test, `ballot-reuse-after-restart` witness. |
