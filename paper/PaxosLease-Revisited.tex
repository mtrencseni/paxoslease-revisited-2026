...
\]
where $D_P$ is the proposer's local duration, $D_A$ is the acceptor's local exclusion duration, and $\gamma \leq 1$ covers the worst case, in which the proposer's clock runs slow and stretches $D_P$ out in real time while the acceptor's runs fast and uses $D_A$ up early.  The arithmetic content of this condition, that bounded rates together with the cross-multiplied duration inequality imply real-time containment, is proved in TLAPS as \code{DriftContainmentArithmetic}.
