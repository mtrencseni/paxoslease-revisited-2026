...
\[
  \frac{D_P}{1-\rho} \leq \frac{D_A}{1+\rho},
\]
where $D_P$ is the proposer's local duration and $D_A$ is the acceptor's local exclusion duration.  The arithmetic content of this condition, that bounded rates together with the cross-multiplied duration inequality imply real-time containment, is discharged by the TLAPS obligation \code{DriftContainmentArithmetic}.
...
We state the algorithm as numbered steps, in the shape of the pseudocode of \cite{ref1} so that the differences are legible against the original, whose statement is internally inconsistent (Section~\ref{sec:audit}, \ref{app:latetimer}).  One discipline governs the statement, and it exists because prose and pseudocode are where the original's inconsistency lived: this pseudocode is not a second normative artifact that can drift from the first.  Every step names, in brackets, the action of \code{spec/PaxosLease.tla} that formalizes it; the module is normative, TLC checks it on every run of the evidence pipeline, and if step and action ever diverge, the action governs.

\emph{State.}  An acceptor stores three volatile variables: the highest ballot promised in the current volatile epoch, the accepted lease instance if one exists, and the local deadline until which that lease excludes conflicting grants.  A proposer stores its current ballot, its phase, the identities of the acceptors that responded in the current attempt, the pending deadline $d$ of that attempt, and, when active, the quorum certificate that established its lease.  A lease instance is identified by $(owner, ballot)$; ballots are globally unique, so the owner is defensive redundancy, but the pair is what release must match.
...
Renewal is a new acquisition by the same owner under a higher ballot, entered at P1, which records the currently active lease's ballot as the attempt's renewal base for the P2 check; the old lease remains the only authority until the new accept quorum is complete, and a failed renewal extends nothing.  Capturing the base at P1 rather than inferring it at each response means a fresh attempt can never be reclassified as a renewal by ownership changing while responses arrive.

\emph{Acceptor.}

\invitem{A1 [\code{DeliverPrepare}]}On \code{Prepare(b)}: if $b \geq$ the promised ballot, set promised $:= b$ and reply with the currently live accepted lease, reporting an expired lease as no lease; otherwise reply rejecting, or not at all.  A rejection is a liveness aid, and to serve T4 it should carry the promised ballot, as ScalienDB's does; safety needs nothing from it.
...
\invitem{L1}Activation is made application-visible through learners.  The owner's own learner installs the absolute deadline $d$; a remote learner may cache the owner with a conservative local expiry, but only the owner acts on the lease, and any remote re-anchoring of durations assumes a delivery-delay bound that must then be stated (Section~\ref{sec:audit}, sixth finding).

\emph{Timing rules.}  The 2012 error survived because a timing rule lived in a figure; the timing rules are therefore stated as named rules, not as properties implied by the steps.
...
PaxosLease is not Paxos, and the two should not be confused.  Paxos solves agreement for log slots, choosing at most one value in each slot \cite{ref2}.  A leader-based Paxos implementation uses a distinguished proposer to reduce the number of message delays, but the safety of Paxos does not require a real-time proof that only one process believes itself to be leader.

A lease can be used to choose the Paxos leader.  This may be useful for performance, for local admission decisions, or for reads whose safety depends on there being a single current owner.  However, the lease does not replace Phase~1 of Paxos.  A process that has just acquired a lease may not know which values were previously accepted or chosen, and it must recover them before proposing new client commands.

The standard use of the lease, and the one Keyspace and ScalienDB implement, is the classic Multi-Paxos leader optimization.  We state the leader lifecycle precisely, in the discipline of Section~\ref{sec:algorithm}: each step names its counterpart in the composition model (\code{spec/PaxosLeasePaxos.tla}) and in the executable Python model, and those artifacts govern.
...
We record the history of this section plainly, because it is the thesis of the paper in miniature.  The $\max(D_P, D_A)$ claim survived an exhaustive model-checking pass, since every configuration in that draft used $D_P = D_A$, where the two hypotheses coincide, and the accompanying TLAPS lemma proved a fact that was true but unnecessary.  The separated-duration configurations, written to distinguish the two hypotheses, refuted the acceptor half of the claim within minutes.  A checked artifact answers exactly the configurations it is asked, so choosing configurations that can distinguish competing hypotheses is part of the method.
...
Second, we built a checked projection of the implementations onto their lease acquisition state machine (\code{spec/PaxosLeaseImpl.tla}) and checked when the accidental repair works.  Under timely timeout dispatch the restart condition is
\[
...
% ---------------------------------------------------------------------------
% Section on LLM-assisted formal methods, commented out for now.  To restore,
% delete the \iffalse ... \fi wrapper and put the Section~\ref{sec:llm} clause
% back into the roadmap paragraph of the introduction.
% ---------------------------------------------------------------------------
\iffalse
